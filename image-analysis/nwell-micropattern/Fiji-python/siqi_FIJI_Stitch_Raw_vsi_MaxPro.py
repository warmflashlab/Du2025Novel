...
def image_name_to_frame_and_time(image):
    ...
    [_, file_n, folder_n, plate_n, well_n, obj_n] = name.split('_')   # split on underscores
    file_n = int(file_n[-3:])
    folder_n = int(folder_n[-1])
    plate_n = int(plate_n[-1])
    well_n = int(well_n[-2:])
    obj_n = int(obj_n[-2:])

    return ((folder_n, plate_n, well_n, obj_n, file_n), 0)
...
def get_extension_in_directory(path, extension):
    images = []
    for entry in os.listdir(path):
        entry_path = os.path.join(path, entry)
        if os.path.isfile(entry_path) and entry.endswith(extension):
...
def get_tiffs_in_directory(path):
    return get_extension_in_directory(path, ".tif")


def get_vsis_in_directory(path):
    return get_extension_in_directory(path, ".vsi")



def get_image_tif_bioformats(filename):
    importer_parameters = [
...
def get_image_vsi_bioformats(filename):
    importer_parameters = [
        "open=[{}]".format(filename),
        "color_mode=Default",
...
def get_image(filename):
    if filename.endswith(".tif"):
        return get_image_tif_bioformats(filename)
    elif filename.endswith(".vsi"):
        return get_image_vsi_bioformats(filename)
    else:
...
def stitch_images(temporary_output, image1, image2):
    ...
    pairwise_parameters = [
        "first_image=[{}]".format(image1_open.getTitle()),
        "second_image=[{}]".format(image2_open.getTitle()),
        ...
        "y=0.0000",
        "z=0.0000",
        "registration_channel_image_1=[Only channel 2]",
        "registration_channel_image_2=[Only channel 2]",
...
def combine_images(temporary_output, image1, image2):
    ...
    concatenate_parameters = [
        ...
        "keep open",
        "image1=[{}]".format(image1_open.getTitle()),
        "image2=[{}]".format(image2_open.getTitle())
        ]
...
BASE_PATH = "/Volumes/Siqi_life_three/ap-axis/micropatterning-live/20250412/live/"
RAW_PATH_NAME = "images_Norm"
...
for folder in get_directories_in_directory(RAW_PATH):
    ...
    for chunk in divide_chunks(images, N_POS_IN_OBJ):
        ...
        (folder_n, plate_n, well_n, obj_n, file_n) = first_image
        image_name = "MaxPro_File{0:03d}_Folder{1:d}_Plate{2:d}_Well{3:02d}_Obj{4:02d}.tif".format(file_n, folder_n, plate_n, well_n, 1 + obj_n // N_POS_IN_OBJ)
        ...
        for i in range(n_times):
            ...
            for next in chunk[1:]:
                ...
                image_name = "i{}_o{}_stitch_{}__{}__{}x{}.tif".format(
                    total_index, object_number, folder_name, previous, next, i
                    )
        ...
        for i, (next_image, czt) in enumerate(zip(to_fuse[1:], czts[1:])):
            image_name = "i{}_o{}_concat_{}_{}_{}.tif".format(
                total_index, object_number, folder_name, chunk_string.replace(', ', '_'), i
                )
            temporary_image_path = os.path.join(TEMPORARY_PATH, image_name)
